...
(defvar cljdoc-alist
  '(("*" . "(* x y & more) Returns the product of nums. (*) returns 1. Does not auto-promote longs, will throw on overflow. See also: *'")
    ...
    ("->ArrayChunk" . "(->ArrayChunk am arr off end) Positional factory function for class clojure.core.ArrayChunk.")
    ("->Vec" . "(->Vec am cnt shift root tail _meta) Positional factory function for class clojure.core.Vec.")
    ("->VecNode" . "(->VecNode edit arr) Positional factory function for class clojure.core.VecNode.")
    ("->VecSeq" . "(->VecSeq am vec anode i offset) Positional factory function for class clojure.core.VecSeq.")
    ...
    ("await-for" . "(await-for timeout-ms & agents) Blocks the current thread until all actions dispatched thus far (from this thread or agent) to the agents have occurred, or the timeout (in milliseconds) has elapsed. Returns logical false if returning due to timeout, logical true otherwise.")
    ...
    ("defn" . "(defn name doc-string? attr-map? ([params*] prepost-map? body) + attr-map?) Same as (def name (fn [params* ] exprs*)) or (def name (fn ([params* ] exprs*)+)) with any doc-string or attrs added to the var metadata. prepost-map defines a map with optional keys :pre and :post that contain collections of pre or post conditions.")
    ...
    ("drop-while" . "(drop-while pred coll) Returns a lazy sequence of the items in coll starting from the first item for which (pred item) returns logical false.")
    ...
    ("ex-data" . "(ex-data ex) Alpha - subject to change. Returns exception data (a map) if ex is an ExceptionInfo. Otherwise returns nil.")
    ("ex-info" . "(ex-info msg map cause) Alpha - subject to change. Create an instance of ExceptionInfo, a RuntimeException subclass that carries a map of additional data.")
    ...
    ("filterv" . "(filterv pred coll) Returns a vector of the items in coll for which (pred item) returns true. pred must be free of side-effects.")
    ...
    ("flatten" . "(flatten x) Takes any nested combination of sequential things (lists, vectors, etc.) and returns their contents as a single, flat sequence. (flatten nil) returns an empty sequence.")
    ...
    ("get-in" . "(get-in m ks not-found) Returns the value in a nested associative structure, where ks is a sequence of keys. Returns nil if the key is not present, or the not-found value if supplied.")
    ...
    ("hash" . "(hash x) Returns the hash code of its argument. Note this is the hash code consistent with =, and thus is different than .hashCode for Integer, Short, Byte and Clojure collections.")
    ...
    ("mapv" . "(mapv f c1 c2 c3 & colls) Returns a vector consisting of the result of applying f to the set of first items of each coll, followed by applying f to the set of second items in each coll, until any one of the colls is exhausted.  Any remaining items in other colls are ignored. Function f should accept number-of-colls arguments.")
    ...
    ("reduce-kv" . "(reduce-kv f init coll) Reduces an associative collection. f should be a function of 3 arguments. Returns the result of applying f to init, the first key and the first value in coll, then applying f to that result and the 2nd key and value, etc. If coll contains no entries, returns init and f is not called. Note that reduce-kv is supported on vectors, where the keys will be the ordinals.")
